...
def cli():
    ## Read configuration
    parser = argparse.ArgumentParser("Draw 2D poses in images or videos, colour-coded on interaction.")
...
if __name__=='__main__':
    ...
    for c, config_path in enumerate(args.configs):
        ...
        for i, (video_path, vipr_path, poslims_path, det2d_path, visualization_path, visualization_fps) in enumerate(zip(video_paths[c], vipr_paths[c], poslims_paths[c], det2d_paths[c], visualization_paths[c], visualization_fpss[c])):
            ...
            position_satisfied = [position_limits(tracklet) for tracklet in human_tracklets.values()]
            position_satisfied = dict(zip(human_tracklets.keys(), position_satisfied))
            ...
            # det2d_config_frames = det2d_frames[np.unique(np.where(np.any(det2d_frames[:,None]==frames_config[None,:], axis=1))[0])]
            ...
            for det2d_frame in tqdm(det2d_config_frames, desc=f"Drawing poses in video {i+1} of {len(video_paths[c])}, configuration {c+1} of {len(args.configs)} ({os.path.split(args.configs[c])[1]})"):
                ...
                ax.set_ylim(frame.shape[0])
                ax.set_xlim(frame.shape[1])
                ...
                interaction_colour = np.array(cats[category]["interaction_colour"])/255
                position_only_colour = np.array(cats[category]["position_only_colour"])/255
                no_interaction_colour = np.array(cats[category]["no_interaction_colour"])/255
                for tracklet_id in tracklet_ids_active: # Iterate over predictions of this category
                    ...
                    if position_satisfied[tracklet_id][det2d_frame-tracklet[det2d.Keys.start]]:
                        if movement_satisfied[tracklet_id][det2d_frame-tracklet[det2d.Keys.start]]: limb_colour = interaction_colour
                        else: limb_colour = position_only_colour
                    else: limb_colour = no_interaction_colour
                    ...
                    conf_text_bbox.x0, conf_text_bbox.x1 = conf_text_bbox.x1, conf_text_bbox.x0
